...
class ActorCritic(nn.Module):
    ...
    def forward(self, inputs):
        ...
        x = F.dropout(F.elu(self.l1(state)), p=0.5)
        x = F.dropout(F.elu(self.l2(x)), p=0.5)
    ...
    def select_action(self, batch_state, batch_act_mask, device):
        ...
        act_mask = torch.ByteTensor(batch_act_mask).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for split_name, dataloader in splits_to_compute:
            ...
            while dataloader.has_next():
                ...
                cur_metrics = {f'{split_name}_loss': loss, f'{split_name}_ploss': ploss, f'{split_name}_vloss': vloss, f'{split_name}_entropy': eloss, f'{split_name}_reward': total_reward, f'{split_name}_iter': step_counter[split_name]}
